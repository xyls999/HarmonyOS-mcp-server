...
_rag = SimpleRAG()
from retrieval_optimizer import HybridRetriever
_hybrid_retriever = HybridRetriever()
from device_solution_knowledge import DeviceSolutionKnowledge, build_state_query
_device_solution_knowledge = DeviceSolutionKnowledge()
...
from security_anomaly_guard import SecurityAnomalyGuard, format_security_response
...
from controller_concurrency_patch import install_controller_concurrency
from scenes.scene_config import SCENE_META, get_scene_commands, match_prepared_scene
from automation.automation_service import AutomationService
from automation.snapshot_builder import build_snapshot
from custom_led_adapter import CustomLedAdapter
from protocol_contract import AdapterRegistry, PROTOCOL_PROFILES, secure_transport_requirements
try:
...
_security_anomaly_guard = SecurityAnomalyGuard()
...
def _configure_controller_concurrency():
    """修正现场控制器固定临时文件在并发设备命令下的竞态。"""
    return install_controller_concurrency(sys.modules.get("central_controller"))


_configure_controller_concurrency()
...
_AI_CONFIG = {
    ...
    "models": {
        ...
        "iflytek": {
            ...
            # The现场 key is provisioned on the Astron coding endpoint; 4.0Ultra
            # returns PathDomainError/10404 there. Keep it as the multimodal
            # provider slot, but use the model actually available to this key.
            "model": os.environ.get("IFLYTEK_MODEL", "astron-code-latest"),
            "maxTokens": 512,
            "temperature": 0.3,
        },
        "astron": {
...
_AI_DOOR_PASSWORD = os.environ.get("A9_AI_DOOR_PASSWORD", "")
...
def _prepared_scene_intent(text: str) -> dict | None:
    scene_id = match_prepared_scene(text)
    if not scene_id:
        return None
    meta = SCENE_META.get(scene_id, {})
    return {
        "type": "scene", "scene_id": scene_id,
        "scene_name": meta.get("name", scene_id),
        "reason": f"匹配预置场景：{meta.get('name', scene_id)}",
    }


def _assistant_item_requires_acknowledgement(item: dict) -> bool:
    """警报弹窗只用于真实安全事件，不复用为 AI 动作审批。"""
    kind = str((item or {}).get("kind", ""))
    severity = str((item or {}).get("severity", "info"))
    return kind in {"security_alert", "guard_incident"} and severity in {"warning", "danger"}


def _format_execution_result(title: str, execution: dict | None) -> str:
    ...
    for item in results:
        ...
        device_name = _DEVICE_NAMES.get(device_id, device_id or "设备")
        state = "成功" if item.get("success") else f"失败：{item.get('error', '设备未确认')}"
        lines.append(f"- {device_name}：{item.get('action', '调整')}，{state}")
    detail = "\n".join(lines) if lines else "- 当前状态无需改变"
    return (
        f"{title}已直接执行。\n"
        f"成功 {int(execution.get('successCount', 0) or 0)} 项，失败 {int(execution.get('failureCount', 0) or 0)} 项。\n"
        f"具体结果：\n{detail}"
    )
...
def _format_intent_plan(intent: dict) -> str:
    ...
    if intent_type == "device_plan":
        status = str(intent.get("status", "当前设备状态已读取"))
        suggestion = str(intent.get("suggestion", "按当前状态执行最小调整"))
        reason = str(intent.get("analysis", "根据实时状态、历史操作和设备能力推导"))
        commands = intent.get("commands", [])
        detail = "\n".join(
            f"- {_DEVICE_NAMES.get(str(c.get('device_id', '')), str(c.get('device_id', '待确认设备')))} → "
            f"{c.get('action', '调整')} {json.dumps(c.get('params', {}), ensure_ascii=False)}"
        ...
        return (
            ...
            f"三、建议原因：{reason}\n"
            f"具体设备计划：\n{detail}\n"
            "执行方式：设备命令已进入有界并发执行，具体成功与失败以执行结果为准。"
        )
    ...
    return (
        ...
        "执行方式：直接进入有界并发设备执行器，并回传逐设备结果。"
    )


def _sleep_request(text: str) -> bool:
    normalized = str(text or "").strip().lower()
    return any(token in normalized for token in ("睡觉", "睡眠", "要睡了", "准备休息", "休息了"))
...
def _is_log_analysis_request(text: str) -> bool:
    """Keep live log analysis deterministic instead of waiting on an upstream model."""
    normalized = str(text or "").strip()
    if not normalized:
        return False
    analysis_terms = ("分析", "汇总", "总结", "趋势", "统计", "报告")
    explicit_log = "日志" in normalized or "请求统计" in normalized
    period = any(term in normalized for term in ("七日", "7日", "当日", "今日"))
    trend_or_stats = any(term in normalized for term in ("趋势", "统计"))
    return ((explicit_log and any(term in normalized for term in analysis_terms))
            or (period and trend_or_stats))


def _requires_deep_analysis(text: str) -> bool:
    """Route reasoning requests around the legacy parser's shallow `query all`."""
    normalized = str(text or "").strip()
    reasoning_terms = ("综合分析", "深入分析", "分析", "评估", "判断原因", "改进建议", "优化建议")
    evidence_terms = ("历史记录", "历史", "上下文", "传感器", "设备状态", "系统", "智能化", "关联")
    return any(term in normalized for term in reasoning_terms) and any(term in normalized for term in evidence_terms)
...
def _build_api_list():
    ...
    return [
        ...
        {"path": "/api/ai/status/check", "methods": ["POST"], "desc": "手动触发一次十分钟状态检测", "auth": "write"},
        {"path": "/api/ai/automation/catalog", "methods": ["GET"], "desc": "100条智能调整规则目录", "auth": "read"},
        {"path": "/api/ai/automation/popups", "methods": ["GET"], "desc": "待展示的联动与报警弹窗", "auth": "read"},
        {"path": "/api/ai/automation/check", "methods": ["POST"], "desc": "立即执行一次规则检测", "auth": "write"},
        {"path": "/api/ai/knowledge/stats", "methods": ["GET"], "desc": "中文设备处理策略知识库统计与来源", "auth": "read"},
        {"path": "/api/ai/knowledge/search", "methods": ["POST"], "desc": "按当前设备能力检索处理策略", "auth": "read"},
        ...
        {"path": "/api/ai/external/config", "methods": ["GET", "POST"], "desc": "外部信息采集开关与设备端自动位置状态", "auth": "write"},
        {"path": "/api/ai/assistant/feedback", "methods": ["POST"], "desc": "助手事件1-10评分与A-D改进选项", "auth": "write"},
        {"path": "/api/ai/assistant/acknowledge", "methods": ["POST"], "desc": "人工确认助手报警弹窗", "auth": "write"},
        ...
        {"path": "/api/custom/scan", "methods": ["POST"], "desc": "按类型扫描并注册自定义设备", "auth": "write"},
        {"path": "/api/custom/register", "methods": ["POST"], "desc": "用户确认后接入扫描结果", "auth": "write"},
        {"path": "/api/custom/discover", "methods": ["POST"], "desc": "兼容旧版自定义设备发现接口", "auth": "write"},
        {"path": "/api/custom/devices", "methods": ["GET"], "desc": "自定义设备注册表", "auth": "read"},
        {"path": "/api/protocols/catalog", "methods": ["GET"], "desc": "协议、安全能力和适配器目录", "auth": "read"},
...
def _knowledge_available_devices() -> dict[str, dict]:
    """Expose only executable capabilities that the D6 gateway actually owns."""
    capabilities = {
        "light_01": ["on", "off", "auto", "set_brightness"],
        "light_02": ["on", "off", "set_brightness"],
        "light_03": ["on", "off", "set_brightness"],
        "light_04": ["on", "off", "set_brightness"],
        "ac_01": ["on", "off", "cool", "dry", "fan"],
        "fan_02": ["on", "off", "set_speed"],
        "curtain_01": ["set_position", "query"],
        "alarm_01": ["on", "off"],
        # Opening the door is intentionally absent.  Knowledge retrieval may
        # explain a door event, but it cannot grant an unlock capability.
        "door_01": ["deny", "alarm", "record", "query"],
    }
    with _STATUS_LOCK:
        current = {key: dict(value) for key, value in _DEVICE_STATUS.items()}
    return {
        device_id: {"actions": actions, "online": bool(current.get(device_id, {}).get("online", False))}
        for device_id, actions in capabilities.items()
    }
...
def _build_runtime_context(user_msg: str) -> str:
    ...
    """
    packet = {"当前时间": _cst_now_str(), "实时状态": {}, "状态策略匹配": [], "近期操作": [], "传感器历史": [], "历史汇报": [], "近期日志": [], "检索命中": []}
    try:
        packet["实时状态"] = _context_live_state()
    except Exception:
        packet["实时状态"] = {"error": "实时状态暂不可用"}
    try:
        state_snapshot = _automation_live_snapshot()
        state_query = build_state_query(state_snapshot)
        if state_query:
            packet["状态策略匹配"] = _device_solution_knowledge.search(
                state_query, available_devices=_knowledge_available_devices(), limit=6,
            )
    except Exception:
        packet["状态策略匹配"] = []
    try:
    ...
    if str(user_msg or "").strip():
        try:
            hits = _hybrid_retriever.retrieve(
                str(user_msg), rag=_rag, context_engine=_context_engine,
                knowledge=_device_solution_knowledge,
                available_devices=_knowledge_available_devices(), limit=12,
            )
            packet["检索命中"] = hits if isinstance(hits, list) else []
            log(f"[RETRIEVAL] hybrid query={str(user_msg)[:48]} hits={len(packet['检索命中'])} "
                f"sources={sorted({source for hit in packet['检索命中'] for source in hit.get('retrievalSources', [])})}")
        except Exception:
            packet["检索命中"] = []
...
def _is_meaningless_speech(text):
    normalized = str(text or "").strip().lower()
    compact = re.sub(r"[\s，。；：！？、,.!?:;]+", "", normalized)
    if compact in {"0", "1", "true", "false", "none", "null", "成功", "已完成", "相关信息"}:
        return True
    return not re.search(r"[\u4e00-\u9fff]", compact)
...
def _tts_speak(text, category="default"):
    ...
    if category in {"external", "external_digest"}:
        log("[TTS] 已阻止外部正文进入播报链路")
        return False
    # 外部信息只允许一个短提示，正文永远不朗读，也不写入状态上下文。
    if category == "external_notice" and _voice_summary(text).strip() != "新闻信息已收集":
        log("[TTS] 已阻止外部通知正文")
        return False
    global _LAST_TTS_TEXT, _LAST_TTS_AT, _TTS_LAST_BY_CATEGORY
    normalized = _voice_summary(text)
    if not normalized or _is_meaningless_speech(normalized):
        log("[TTS] 已过滤无效播报")
        return False
    now = time.monotonic()
...
def _voice_bridge_status():
    ...
    if status.get("running") and status.get("pid"):
        try:
            ...
            command_line = Path(f"/proc/{pid}/cmdline").read_bytes().decode("utf-8", "ignore")
            if "voice_bridge_d6.py" not in command_line:
                status["running"] = False
        except Exception:
            status["running"] = False
    return status
...
def _voice_config():
    ...
    serial_port = str(configured.get("serialPort") or os.environ.get("A9_VOICE_SERIAL", "auto")).strip()
    auto_mode = serial_port.lower() in ("auto", "自动识别")
    enabled = bool(configured.get("enabled", True)) and bool(serial_port)
    status = _voice_bridge_status()
    return {
        "enabled": enabled,
        "serialPort": "自动识别" if auto_mode else serial_port,
        "autoDetect": auto_mode,
        "candidatePorts": status.get("candidatePorts", ["/dev/ttyS3", "/dev/ttyS4", "/dev/ttyS8"] if auto_mode else []),
        "detectedPort": status.get("detectedPort", ""),
...
def _stop_voice_bridge():
    ...
    # 自动识别模式由 shell 脱离启动，按状态文件回收真正的桥接进程。
    try:
        status = json.loads(VOICE_STATUS_PATH.read_text("utf-8"))
        status_pid = int(status.get("pid", 0) or 0)
        if status_pid and status_pid != os.getpid():
            try:
                command_line = Path(f"/proc/{status_pid}/cmdline").read_bytes().decode("utf-8", "ignore")
            except Exception:
                command_line = ""
            if "voice_bridge_d6.py" in command_line:
                os.kill(status_pid, 15)
    except Exception:
        pass
...
def _voice_bridge_command(serial_port, gateway):
    """在 D6 便携 Python 环境中通过同一 musl loader 启动桥接。"""
    loader = SMART_HOME_DIR.parent / "python-portable" / "lib" / "ld-musl-armhf.so.1"
    # 进程本身由 loader 启动时，sys.executable 可能指向 loader 而不是
    # Python ELF；优先使用设备运行时的固定 Python 路径。
    python_bin = SMART_HOME_DIR.parent / "python-portable" / "usr" / "bin" / "python3.14"
    if not python_bin.exists():
        python_bin = Path(sys.executable)
    if loader.exists() and python_bin.exists():
        library_path = SMART_HOME_DIR.parent / "python-portable" / "lib"
        usr_library_path = SMART_HOME_DIR.parent / "python-portable" / "usr" / "lib"
        argv = [str(loader), "--library-path", f"{library_path}:{usr_library_path}:/system/lib",
                str(python_bin), str(VOICE_BRIDGE_PATH), "--port", serial_port,
                "--gateway", gateway]
        # D6 的便携 Python 需要由 musl loader 进入；经 /system/bin/sh exec
        # 后与开机脚本使用完全相同的加载路径，避免 Popen 直接 exec 丢失解释器环境。
        return ["/system/bin/sh", "-c", "exec " + " ".join(shlex.quote(item) for item in argv)]
    return [sys.executable, str(VOICE_BRIDGE_PATH), "--port", serial_port, "--gateway", gateway]


def _ensure_voice_bridge():
    global _voice_bridge_process
    config = _read_voice_json()
    serial_port = str(config.get("serialPort") or os.environ.get("A9_VOICE_SERIAL", "auto")).strip()
    if bool(config.get("enabled", True)) and not serial_port:
        serial_port = "auto"
        config["serialPort"] = serial_port
        _write_voice_json(config)
    if not bool(config.get("enabled", True)) or not serial_port:
        _stop_voice_bridge()
        return _voice_config()
    with _VOICE_BRIDGE_LOCK:
        if _voice_bridge_process is not None and _voice_bridge_process.poll() is None:
            return _voice_config()
        active_status = _voice_bridge_status()
        ...
        try:
            command = _voice_bridge_command(serial_port, f"http://127.0.0.1:{PORT}")
            auto_mode = serial_port.lower() in ("auto", "自动识别") and len(command) >= 3 and command[0] == "/system/bin/sh"
            if auto_mode:
                # OpenHarmony 从网关进程直接 exec 便携 Python 偶发返回 -11；
                # 交给同一 shell 后台启动，加载路径与 run_v6.sh 完全一致。
                launch = command[2].replace("exec ", "", 1) + \
                    f" >>{shlex.quote(str(log_path))} 2>&1 &"
                launcher_process = subprocess.Popen(
                    ["/system/bin/sh", "-c", launch], cwd=str(SMART_HOME_DIR), env=env,
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                )
                # shell 仅负责拉起脱离的桥接进程；真正生命周期由状态文件维护。
                launcher_process.wait(timeout=1)
                _voice_bridge_process = None
                stream.close()
            else:
                _voice_bridge_process = subprocess.Popen(
                    command, cwd=str(SMART_HOME_DIR), env=env, stdout=stream, stderr=stream,
                )
            log(f"[VOICE] D6串口桥接已启动，等待端口: {serial_port}")
            time.sleep(0.15)
            if not auto_mode and _voice_bridge_process.poll() is not None:
                log(f"[VOICE] D6串口桥接退出，返回码: {_voice_bridge_process.returncode}")
                config["enabled"] = False
...
def _set_voice_enabled(enabled, serial_port=None):
    config = _read_voice_json()
    provided_port = str(serial_port).strip() if serial_port is not None else ""
    if provided_port:
        if provided_port.lower() not in ("auto", "自动识别") and (not provided_port.startswith("/dev/") or len(provided_port) <= 5):
            return {"success": False, "error": "串口路径必须是明确的设备路径，例如 /dev/ttyS3"}
        config["serialPort"] = "auto" if provided_port.lower() in ("auto", "自动识别") else provided_port
    elif enabled and not str(config.get("serialPort") or os.environ.get("A9_VOICE_SERIAL", "")).strip():
        # 前端不暴露物理路径；启用时由后端在现场候选端口中自动识别。
    ...
    if enabled:
        ...
        if not result.get("running") and str(current_port).lower() in ("auto", "自动识别"):
            # 自动模式的后台 shell 与桥接进程是两步启动，给状态文件一个短暂落盘窗口。
            for _ in range(6):
                time.sleep(0.2)
                result = _voice_config()
...
def db_init():
    ...
    # Context, proactive checks and device callbacks write concurrently. WAL
    # keeps readers available while a writer commits; the busy timeout avoids
    # transient failures surfacing as failed AI turns.
...
def _log_security(result, input_text, source="chat"):
    try:
        ...
        _notify_qq_async("security_alert", "AI安全警戒", f"检测到{result.category}风险，已阻止并记录：{result.reason}", {"rule": result.rule_id, "source": source})
    except Exception as e:
...
def _log_security_auth(rule_id, input_text, reason, source="api"):
    try:
        ...
        _notify_qq_async("security_alert", "重要登录安全事件", reason[:200], {"rule": rule_id, "source": source})
    except Exception as e:
        log(f"[AUTH-SEC] log error: {e}")


def _publish_security_anomaly(event: dict) -> None:
    """Persist and fan out one deterministic anomaly decision."""
    ...
    try:
        conn = _db()
        conn.execute(
            "INSERT INTO security_events(rule_id,severity,category,input_text,matched_text,reason,source,blocked) "
            "VALUES(?,?,?,?,?,?,?,?)",
            (rule_id, severity, "intrusion_detection", "", "", message, "security_anomaly_guard", 1),
        )
        conn.commit()
        conn.close()
    except Exception as exc:
        log(f"[SECURITY] persist failed: {type(exc).__name__}")
    buzzer_success = not bool(event.get("activateBuzzer"))
    if event.get("activateBuzzer"):
        try:
            _security_anomaly_guard.authorize_state("alarm_01", True, source="security_guard")
            result = hw_toggle("alarm_01", True)
            buzzer_success = bool(result.get("success"))
            if result.get("success"):
                _apply_commanded_device_state("alarm_01", True, 1)
            _tts_speak("安全警报，检测到疑似入侵行为，已开启蜂鸣器并通知主人", category="security")
        except Exception as exc:
            buzzer_success = False
            log(f"[SECURITY] buzzer action failed: {type(exc).__name__}")
    preliminary = format_security_response(event, buzzer_success=buzzer_success, qq_queued=False)
    qq_queued = _notify_qq_async(
        preliminary["notificationEvent"], "智慧家居安全警报", message,
        {"ruleId": rule_id, "severity": severity, "evidence": evidence},
    )
    response = format_security_response(event, buzzer_success=buzzer_success, qq_queued=qq_queued)
    _record_context_event(
        "security_anomaly", message,
        details={"ruleId": rule_id, "severity": severity, "evidence": evidence,
                 "operations": response["operations"]},
        source="security_anomaly_guard",
        severity="critical" if severity == "Critical" else "warning",
    )
    if _proactive_intelligence is not None:
        try:
            _proactive_intelligence.create_report(
                "security_alert", "安全警戒", message, evidence,
                response["operations"],
                {"labels": ["异常置信度", "重复频率", "影响范围"],
                 "values": [100 if severity == "Critical" else 70,
                            int(evidence.get("attempts", evidence.get("changesInTwoMinutes", 1))) * 20,
                            100 if evidence.get("deviceId") == "door_01" or rule_id.startswith("door.") else 55],
                 "types": ["radar", "pie", "line"]},
                severity="danger" if severity == "Critical" else "warning",
            )
        except Exception as exc:
            log(f"[SECURITY] assistant report failed: {type(exc).__name__}")
    if _proto_gateway:
        _proto_gateway.broadcast_event("security_alert", {
            "rule_id": rule_id, "severity": severity, "reason": message,
            "evidence": evidence, "automatic": True,
        })
            ...
            with _ALARM_LOCK:
                if _adaptive_guard is None and alarm_now == 1 and _last_kitchen_alarm == 0:
                    ...
                    if _LINKAGE_CONFIG.get("kitchen_alarm_buzzer", {}).get("enabled", True):
                        _security_anomaly_guard.authorize_state("alarm_01", True, source="kitchen_linkage")
                        result = hw_toggle("alarm_01", True)
                    ...
                    if _LINKAGE_CONFIG.get("kitchen_alarm_exhaust", {}).get("enabled", False):
                        _security_anomaly_guard.authorize_state("fan_02", True, source="kitchen_linkage")
                    ...
                    if _LINKAGE_CONFIG.get("kitchen_alarm_buzzer", {}).get("clear_on_recovery", True):
                        _security_anomaly_guard.authorize_state("alarm_01", False, source="kitchen_recovery")
                        hw_toggle("alarm_01", False)
                        _tts_speak("厨房报警已恢复，警报已关闭")
                        log("[ALARM-LINKAGE] 厨房报警恢复 alarm=0 → BEEP OFF")
                    # 报警恢复 -> 关换气扇 (受配置)
                    if _LINKAGE_CONFIG.get("kitchen_alarm_exhaust", {}).get("enabled", False):
                        _security_anomaly_guard.authorize_state("fan_02", False, source="kitchen_recovery")
    ...
    try:
        ...
        if r["success"]:
            ...
            with _STATUS_LOCK:
                ...
                radar_enabled = bool(_LINKAGE_CONFIG.get("radar_presence", {}).get("enabled", True))
                radar_ready = d.get("radar_uart_ready", 0) == 1
                radar_present = bool(d.get("radar_target_present", 0))
                _SENSOR_STATUS["radar_01"] = {
                    "value": 1 if radar_present else 0,
                    "present": radar_present if radar_enabled and radar_ready else None,
                    "radar_target_present": 1 if radar_present else 0,
                    "radar_distance_cm": d.get("radar_distance_cm", 0),
                    "online": bool(radar_enabled and radar_ready),
                    "unit": "有人" if radar_present else "无人",
                    "ts": time.time(),
                }
            _process_adaptive_guard()
            log(f"[POLL] 卫生间在线 brightness={d.get('light_brightness',0)} motor={d.get('motor_running',0)} speed={d.get('motor_speed',0)} radar={d.get('radar_target_present',0)}")
        else:
            with _STATUS_LOCK:
                _DEVICE_STATUS["light_04"] = {"online": False}
                _DEVICE_STATUS["fan_02"] = {"online": False}
                _SENSOR_STATUS["radar_01"] = {"online": False, "present": None, "ts": time.time()}
    except Exception as e:
...
def _security_state_watch_thread():
    """Compare polled state with gateway-authorized mutations once per second."""
    while True:
        try:
            with _STATUS_LOCK:
                kitchen_alarm_active = any(
                    bool(_SENSOR_STATUS.get(sensor_id, {}).get("is_alert", False))
                    for sensor_id in ("smoke_01", "heat_01", "air_01")
                )
                snapshot = {
                    str(device_id): bool(state.get("is_on", False))
                    for device_id, state in _DEVICE_STATUS.items()
                    if state.get("online") is True and "is_on" in state
                }
            for device_id, current_state in snapshot.items():
                if device_id in {"alarm_01", "fan_02"} and not current_state and not kitchen_alarm_active:
                    _security_anomaly_guard.authorize_state(
                        device_id, False, source="sensor_recovery"
                    )
                event = _security_anomaly_guard.observe_state(device_id, current_state)
                if event:
                    _publish_security_anomaly(event)
        except Exception as exc:
            log(f"[SECURITY] state watch failed: {type(exc).__name__}")
        time.sleep(1)
            ...
            if _adaptive_guard is not None:
                ...
                with _ALARM_LOCK:
                    if _last_kitchen_alarm == 0:
                        ...
                        if _LINKAGE_CONFIG.get("kitchen_alarm_buzzer", {}).get("enabled", True):
                            _security_anomaly_guard.authorize_state("alarm_01", True, source="kitchen_udp")
                            hw_toggle("alarm_01", True)
                            _tts_speak("厨房UDP报警，已触发警报")
                        if _LINKAGE_CONFIG.get("kitchen_alarm_exhaust", {}).get("enabled", True):
                            _security_anomaly_guard.authorize_state("fan_02", True, source="kitchen_udp")
...
## 主动触发边界
- 主动自动化只允许十余项已确认核心策略：温度、湿度、厨房烟雾/热敏、夜间窗帘、凌晨照明、毫米波无人关闭设备、低照度有人照明、门禁与安全攻击。
- 禁止自行创造场景、阈值、设备能力或自动执行规则；白名单以外只能给出建议，不得生成设备控制指令。
- 上述限制只约束后台主动触发；主人明确发出的设备控制命令仍按实时状态和设备能力执行。
...
2. 一个回复可以包含多个指令
3. 不确定时不要输出指令，只回复文字
4. 设备离线时不要伪报成功；仍可输出控制指令交给执行器返回准确失败原因
5. AI 门禁开门由服务端使用受保护密码执行；不得在回复、日志或指令参数中泄露密码
6. 用户的模糊意图（"有点暗""太热了""我想看电影"）必须结合实时状态、设备能力、预置场景和历史上下文生成可解释的设备计划，并直接执行，不等待确认
7. 计划必须列出具体设备、动作和依据；多个设备用并发指令直接执行，随后根据结构化执行结果如实说明每项成功或失败
8. 对可能表达家庭需求的短语不要只回复“没理解”；证据足够时直接执行最小、安全、可逆的计划，证据不足时只问一个最关键的澄清问题
9. 你可以调用API获取信息或修改配置，使用<<CALL:>>指令
10. 你可以往KV知识库写入自定义知识供后续对话使用
11. 判断联动是否开启时必须以 effective_enabled 为准；enabled 只是旧配置值，二者冲突时不得用 enabled 推断实际运行状态
"""
...
def _context_live_state():
    ...
    if _intent_engine:
        try:
            ...
            pass
    if _custom_led_adapter is not None:
        for custom in _custom_led_adapter.list_devices():
            device_id = custom.get("id")
            if not device_id or device_id in known_device_ids:
                continue
            devices.append({
                "id": device_id, "name": custom.get("name", device_id),
                "type": "custom", "room": custom.get("room", ""),
                "online": custom.get("status") == "online",
                "isOn": bool(custom.get("isOn", False)),
                "primaryValue": custom.get("primaryValue"),
                "capabilities": custom.get("capabilities", []), "custom": True,
            })
            known_device_ids.add(device_id)
    with _LINKAGE_LOCK:
    ...
    if "temp_humidity_ac" in linkage:
        linkage["temp_humidity_ac"]["effective_enabled"] = bool(
            master_enabled and (temperature_config.get("enabled", False) or humidity_config.get("enabled", False))
        )
    if "kitchen_alarm_buzzer" in linkage:
        linkage["kitchen_alarm_buzzer"]["effective_enabled"] = bool(
            master_enabled and kitchen_config.get("enabled", True) and kitchen_config.get("buzzer", True)
        )
    if "kitchen_alarm_exhaust" in linkage:
        linkage["kitchen_alarm_exhaust"]["effective_enabled"] = bool(
            master_enabled and kitchen_config.get("enabled", True) and kitchen_config.get("exhaust", True)
        )
    if "living_light_auto" in linkage:
        linkage["living_light_auto"]["effective_enabled"] = bool(
            master_enabled and linkage["living_light_auto"].get("enabled", True)
        )
...
def _guard_execute(action):
    ...
    if command in ("on", "off"):
        expected = command == "on"
        _security_anomaly_guard.authorize_state(device_id, expected, source="adaptive_guard")
        result = hw_toggle(device_id, expected)
        if result.get("success"):
            _apply_commanded_device_state(device_id, expected)
        return result
...
def _automation_execute(command):
    """把规则引擎命令接到现有设备执行器；门禁始终禁止自动开门。"""
    device_id = str(command.get("device_id", ""))
    action = str(command.get("action", ""))
    if device_id == "door_01":
        return {"success": False, "error": "智能规则禁止自动操作门禁"}
    if action == "record":
        return {"success": True, "recorded": True}
    try:
        params = dict(command.get("params") or {})
        if device_id == "light_01" and action == "auto":
            result = set_living_light_auto(True)
            if result.get("success"):
                with _STATUS_LOCK:
                    state = dict(_DEVICE_STATUS.get(device_id, {}))
                    state["auto_mode"] = True
                    state["online"] = True
                    state["ts"] = time.time()
                    _DEVICE_STATUS[device_id] = state
            return result
        if action in {"on", "off"}:
            expected = action == "on"
            result = hw_toggle(device_id, expected)
            if result.get("success"):
                _apply_commanded_device_state(device_id, expected, params.get("value"))
            return result
        result = hw_control(device_id, action, params)
        if result.get("success") and action == "set_position":
            _apply_commanded_device_state(device_id, True, params.get("value"))
        return result
    except Exception as exc:
        return {"success": False, "error": str(exc)[:240]}


def _automation_live_snapshot():
    """从同一把状态锁复制一份原子快照，避免规则看到半次轮询数据。"""
    now = _cst_now()
    with _STATUS_LOCK:
        devices = {key: {**value, "observed_at": value.get("ts", now.timestamp())}
                   for key, value in _DEVICE_STATUS.items()}
        sensors = {key: {**value, "observed_at": value.get("ts", now.timestamp())}
                   for key, value in _SENSOR_STATUS.items()}
    # Light level is owned by the persistent sensor adapter rather than an
    # area controller.  Include its latest value so A011 has a real input.
    try:
        conn = _db()
        row = conn.execute("SELECT current_value,is_alert FROM sensors WHERE id='light_s_01'").fetchone()
        conn.close()
        if row is not None and row[0] is not None:
            sensors["light_s_01"] = {
                "value": float(row[0]), "is_alert": bool(row[1]),
                "online": True, "observed_at": now.timestamp(),
            }
    except Exception:
        pass
    snapshot = build_snapshot(devices, sensors, {}, now=now, device_ttl_seconds=180, sensor_ttl_seconds=420)
    snapshot["time"] = {"hour": now.hour, "minute": now.minute}
    return snapshot


def _run_automation_check(manual=False):
    if _automation_service is None:
        return {"capturedAt": _cst_now().isoformat(), "executed": [], "skipped": [], "popups": [], "error": "automation unavailable"}
    # A001-A005/A018-A020 已由自适应安全护栏负责；避免重复执行，其他核心规则由新引擎统一处理。
    enabled = _automation_service.enabled_rule_ids(["A006", "A008", "A009", "A010", "A011"])
    snapshot = _automation_live_snapshot()
    result = _automation_service.evaluate(snapshot, enabled_rule_ids=enabled)
    state_query = build_state_query(snapshot)
    result["knowledgeMatches"] = (
        _device_solution_knowledge.search(
            state_query, available_devices=_knowledge_available_devices(), limit=6,
        ) if state_query else []
    )
...
def _guard_snapshot(extra_kitchen=None):
    ...
    with _STATUS_LOCK:
        ...
        for definition in DEVICE_DEFS:
            device_id = str(definition.get("id", ""))
            cached = dict(_DEVICE_STATUS.get(device_id, {}))
            devices.append({"id": device_id, "name": definition.get("name", device_id),
                            "type": definition.get("type", ""), "room": definition.get("room", ""),
                            "isOn": bool(cached.get("is_on", False)),
                            "online": bool(cached.get("online", False))})
        radar = dict(_SENSOR_STATUS.get("radar_01", {}))
        if radar:
            sensors.append({
                "id": "radar_01", "type": "mmwave", "room": "卫生间",
                "value": radar.get("value"), "presence": radar.get("present"),
                "isAlert": False, "online": bool(radar.get("online", False)),
            })
    ...
    try:
        ...
        pass
    return {"sensors": sensors, "devices": devices}
...
def _process_adaptive_guard(extra_kitchen=None):
    if _adaptive_guard is None:
        return []
    try:
        incidents = _adaptive_guard.process_snapshot(_guard_snapshot(extra_kitchen))
        if _automation_service is not None:
            for incident in incidents:
                try:
                    _automation_service.publish_guard_incident(incident)
                except Exception as bridge_error:
                    log(f"[GUARD] popup bridge failed: {type(bridge_error).__name__}")
        return incidents
    except Exception as exc:
...
def _publish_device_operation(device_id, action_text, success):
    ...
    if _proactive_intelligence is None:
        return None
    try:
        result = _proactive_intelligence.publish_operation(device_id, action_text, bool(success))
        device_name = _DEVICE_NAMES.get(str(device_id), str(device_id))
        _notify_qq_async("device_operation", "设备状态变化", f"{device_name}{action_text}{'成功' if success else '失败'}",
                         {"deviceId": device_id, "success": bool(success)})
        return result
    except Exception as exc:
...
def _notify_qq_async(event_type: str, title: str, message: str, extra: dict | None = None) -> bool:
    """所有重要事件统一进入已配置的 QQ/Qmsg 网关，异步且带服务端冷却。"""
    if _notification_service is None or not _notification_service.status().get("configured"):
        return False
    threading.Thread(target=_notification_service.send,
                     args=(event_type, title, message, extra or {}), daemon=True).start()
    return True
...
def _apply_commanded_device_state(device_id, is_on, primary_value=None):
    ...
    _security_anomaly_guard.authorize_state(str(device_id), bool(is_on), source="gateway")
    with _STATUS_LOCK:
...
def _initialize_adaptive_guard():
    global _adaptive_guard, _notification_service, _proactive_intelligence, _automation_service
    try:
    ...
    _adaptive_guard.update_config({"planConfirmation": {"enabled": False}})
    _proactive_intelligence = ProactiveIntelligence(DB_PATH)
    _automation_service = AutomationService(
        DB_PATH,
        executor=_automation_execute,
        speaker=lambda text, category="automation": _tts_speak(text, category=category),
        context_recorder=_record_context_event,
    )
    try:
        # Open incidents may be much older than the latest history page (for
        # example one physical heat alarm that has remained active for days).
        # Put them first so startup restores the canonical event instead of a
        # recent duplicate or omitting it entirely.
        guard_incidents = _adaptive_guard.list_incidents(limit=200, pending_only=True)
        known_incident_ids = {int(item.get("id", 0)) for item in guard_incidents}
        guard_incidents.extend(
            item for item in _adaptive_guard.list_incidents(limit=200, pending_only=False)
            if int(item.get("id", 0)) not in known_incident_ids
        )
        sync_result = _automation_service.sync_guard_incidents(
            guard_incidents
        )
        log(f"[GUARD] restored open incidents to popup outbox: {sync_result}")
    except Exception as exc:
        log(f"[GUARD] startup popup sync failed: {type(exc).__name__}")
...
def _run_proactive_cycle_now(manual: bool = False):
    """执行一次与十分钟周期相同的状态汇总，供定时器和手动按钮共用。"""
    if _proactive_intelligence is None:
        return []
    automation_result = _run_automation_check(manual=manual)
    if manual:
        reports = _proactive_intelligence.run_cycle(force=True)
    else:
        reports = _proactive_intelligence.run_cycle()
    for report in reports:
        ...
        if report.get("kind") == "external_digest":
            # External information is visible in the assistant feed only;
            # it never enters state context and never reaches TTS.
            continue
        if report.get("severity") in {"warning", "danger"}:
            _tts_speak(report.get("summary", "检测到需要关注的设备行为"))
    if automation_result.get("executed"):
        reports.append({
            "id": int(time.time() * 1000),
            "kind": "automation_adjustment",
            "title": "智能联动调整",
            "summary": f"已执行 {len(automation_result['executed'])} 条规则调整。",
            "severity": "warning" if any(item.get("urgent") for item in automation_result["executed"]) else "info",
            "evidence": {"automation": automation_result},
        })
    meaningful_reports = [
        report for report in reports
        if report.get("severity") in {"warning", "danger"}
        or report.get("kind") == "automation_adjustment"
    ]
    if meaningful_reports:
        _notify_qq_async(
            "status_summary", "十分钟家庭状态汇报",
            f"发现 {len(meaningful_reports)} 条变化或调整，请查看助手详情。",
            {"周期": "十分钟"},
        )
    else:
        log("[PROACTIVE] 本周期无变化，已跳过无效 QQ 与语音播报")
    return reports


def _run_external_cycle_now():
    """独立运行十分钟外部信息摘要，不写入状态上下文，只允许前端播报短提示。"""
    ...
    with _external_digest_lock:
        ...
        _external_digest_at = time.time()
    # 外部信息只通过 /api/ai/external/feed 在助手顶部展示。它不写入
    # assistant_proactive_reports 或 ai_context_events，避免进入状态/报警上下文。
    try:
        conn = _db()
        conn.execute("DELETE FROM assistant_proactive_reports WHERE kind='external_digest'")
        conn.commit()
        conn.close()
    except Exception:
        pass
    item_count = len(result.get("items", []))
    _notify_qq_async("external_digest", "外部信息状态汇报", f"新闻信息已收集，共 {item_count} 条；正文不播报。", {"周期": "十分钟"})
    log(f"[EXTERNAL] 十分钟摘要已更新，条目={item_count}，正文不播报、不进入状态上下文")
    return result


def _external_cycle_thread():
    next_run = time.monotonic()
    while True:
        delay = max(0.0, next_run - time.monotonic())
        if delay:
            time.sleep(delay)
        next_run += float(_EXTERNAL_DIGEST_INTERVAL)
        try:
            _run_external_cycle_now()
        except Exception as exc:
            log(f"[EXTERNAL] 十分钟摘要失败: {type(exc).__name__}")


def _proactive_cycle_thread():
    """Run deterministic proactive summaries every ten minutes."""
    next_run = time.monotonic() + 600.0
    while True:
        delay = max(0.0, next_run - time.monotonic())
        if delay:
            time.sleep(delay)
...
def _door_password_required_for_action(action):
    """门禁只有开门需要密码；关门是可逆的安全收拢动作。"""
    return str(action or "").strip().lower() == "open"


_DOOR_CLOSE_LOCK = threading.Lock()


def _close_door_without_password():
    """调用桥接层的关门原语，兼容旧版下位机桥接对 close 误加的密码校验。"""
    bridge_globals = getattr(hw_toggle, "__globals__", {})
    living_door = bridge_globals.get("living_door")
    if not callable(living_door):
        return hw_toggle("door_01", False)
    config = bridge_globals.get("_CONFIG")
    timeout = bridge_globals.get("_TIMEOUT", 3.0)
    controller_globals = getattr(living_door, "__globals__", {})
    # 现场存在两代 central_controller：旧版调用 verify_door_password，
    # 新版调用 verify_door_password_explicit。只在 close 临界区替换实际
    # 存在的校验器，保证 open 仍然必须校验密码。
    verifier_names = ("verify_door_password", "verify_door_password_explicit")
    verifiers = {name: controller_globals.get(name) for name in verifier_names}
    try:
        # 只在本次 close 调用的极短临界区内放行，避免改动持久配置或绕过 open。
        with _DOOR_CLOSE_LOCK:
            for verifier_name, verifier in verifiers.items():
                if callable(verifier):
                    controller_globals[verifier_name] = lambda _config, _password=None: True
            raw = living_door(config, "close", timeout, password=None)
    except Exception as exc:
        return {"success": False, "error": str(exc)}
    finally:
        for verifier_name, verifier in verifiers.items():
            if callable(verifier):
                controller_globals[verifier_name] = verifier
    normalize = bridge_globals.get("_hw_ok")
    return normalize(raw) if callable(normalize) else raw
...
class H(BaseHTTPRequestHandler):
    ...
    def _require_auth(self, required_permission="read"):
        ...
        if err:
            event = _security_anomaly_guard.record_auth_failure(
                self.client_address[0], endpoint=self.path.split("?", 1)[0]
            )
            if event:
                _publish_security_anomaly(event)
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif p == "/api/custom/devices":
                auth = self._require_auth("read")
                if auth:
                    custom_devices = _custom_led_adapter.list_devices() if _custom_led_adapter else []
                    self._j(200, {"success": True, "devices": [self._public_custom_device(item) for item in custom_devices]})
            ...
            elif p == "/api/ai/knowledge/stats":
                auth = self._require_auth("read")
                if auth:
                    self._j(200, {"success": True, **_device_solution_knowledge.get_stats()})
                    self._log_remote_access(auth, p)
            elif p == "/api/stats":
                auth = self._require_auth("read")
            ...
            elif p == "/api/ai/automation/catalog":
                auth = self._require_auth("read")
                if auth: self._j(200, self._get_automation_catalog()); self._log_remote_access(auth, p)
            elif p == "/api/ai/automation/popups":
                auth = self._require_auth("read")
                if auth:
                    query = parse_qs(urlsplit(self.path).query)
                    limit = min(100, max(1, int(query.get("limit", ["50"])[0])))
                    self._j(200, self._get_automation_popups(limit)); self._log_remote_access(auth, p)
            ...
            elif p == "/api/protocols/catalog":
                auth = self._require_auth("read")
                if auth:
                    catalog = _adapter_registry.catalog()
                    catalog["runtime"] = _proto_gateway.get_status() if _proto_gateway else {"http": {"status": "running", "port": 8080}}
                    catalog["remotePolicy"] = {
                        name: secure_transport_requirements(name, remote=True)
                        for name in PROTOCOL_PROFILES
                    }
                    self._j(200, catalog)
                    self._log_remote_access(auth, p)
        ...
        try:
            ...
            # ===== 业务接口 (需认证) =====
                ...
                if auth:
                    ...
                    self._log_remote_access(auth, p)
                return
            elif p == "/api/ai/automation/check":
                auth = self._require_auth("write")
                if auth:
                    self._j(200, self._run_automation_check_api(body))
                    self._log_remote_access(auth, p)
                return
            elif p == "/api/ai/knowledge/search":
                auth = self._require_auth("read")
                if auth:
                    self._j(200, self._search_device_solutions(body))
                    self._log_remote_access(auth, p)
                return
            elif p == "/api/ai/external/config":
                auth = self._require_auth("write")
                ...
                return
            elif p == "/api/ai/push/test":
                auth = self._require_auth("write")
                if auth: self._j(200, self._push_test(body)); self._log_remote_access(auth, p)
                return
            elif p == "/api/ai/offline/config":
                auth = self._require_auth("write")
                if auth:
            ...
            # ===== AI 智能意图接口 (POST) =====
                ...
                return
            elif p == "/api/custom/register":
                auth = self._require_auth("write")
                if auth:
                    result = self._register_scanned_custom(body)
                    self._j(200 if result.get("success") else 400, result)
                    self._log_remote_access(auth, p, 200 if result.get("success") else 400)
                return
            elif p in ("/api/custom/scan", "/api/custom/discover"):
                auth = self._require_auth("write")
                if auth:
                    result = self._discover_custom_led(body)
                    self._j(200 if result.get("success") else 400, result)
                    self._log_remote_access(auth, p, 200 if result.get("success") else 400)
                return
                ...
                if auth:
                    ...
                    self._log_remote_access(auth, p)
                return
            elif p == "/api/ai/assistant/acknowledge":
                auth = self._require_auth("write")
                if auth:
                    try:
                        self._j(200, self._acknowledge_assistant_alert(body))
                    except (TypeError, ValueError) as exc:
                        self._j(400, {"success": False, "error": str(exc), "code": 400})
                    self._log_remote_access(auth, p)
                return
            ...
            if m:
                ...
                if auth:
                    device_id = m.group(1)
                    if _custom_led_adapter is not None and _custom_led_adapter.get(device_id):
                        params = body.get("params") if isinstance(body.get("params"), dict) else body
                        result = _custom_led_adapter.control(device_id, body.get("action", "toggle"), params)
                        result = self._record_custom_led_operation(device_id, body.get("action", "toggle"), params, result)
                        self._j(200 if result.get("success") else 400, self._public_custom_payload(result))
                    else:
                        self._j(200, self._control_device(device_id, body))
                    self._log_remote_access(auth, p)
                return
            ...
            if m:
                ...
                if auth:
                    device_id = m.group(1)
                    if _custom_led_adapter is not None and _custom_led_adapter.get(device_id):
                        result = _custom_led_adapter.control(device_id, "on" if body.get("isOn", body.get("is_on", False)) else "off", body)
                        result = self._record_custom_led_operation(device_id, "toggle", body, result)
                        self._j(200 if result.get("success") else 400, self._public_custom_payload(result))
                    else:
                        self._j(200, self._toggle_device(device_id, body))
                    self._log_remote_access(auth, p)
                return
    ...
    def _get_devices(self):
        ...
        if _intent_engine:
            try:
                ...
                pass
        # 自定义 LED 由独立适配器维护；适配器只保存能力和最近一次状态，
        # 控制后会立即回写，因此设备页不会把自定义设备误降级成 Mock。
        if _custom_led_adapter is not None:
            for custom in _custom_led_adapter.list_devices():
                custom_id = custom.get("id")
                if custom_id in existing_ids:
                    # IntentEngine 也会注册模板；真实适配器状态优先覆盖模板状态。
                    for index, current in enumerate(result):
                        if current.get("id") == custom_id:
                            result[index] = self._public_custom_device(custom)
                            break
                    continue
                result.append(self._public_custom_device(custom))
                existing_ids.add(custom_id)
        return _visible_devices(result)

    @staticmethod
    def _public_custom_device(device):
        """前端只拿能力和状态；适配器 endpoint/IP 永不进入公开设备列表。"""
        public = dict(device or {})
        public.pop("endpoint", None)
        public.pop("board_ip", None)
        return public

    @staticmethod
    def _public_custom_payload(value):
        """递归删除适配器响应中的内网地址和凭据字段。"""
        sensitive = {"endpoint", "board_ip", "ip", "api_key", "token", "authorization"}
        if isinstance(value, dict):
            return {
                key: H._public_custom_payload(item)
                for key, item in value.items()
                if str(key).lower() not in sensitive
            }
        if isinstance(value, list):
            return [H._public_custom_payload(item) for item in value]
        return value

    def _discover_custom_led(self, body):
        if _custom_led_adapter is None:
            return {"success": False, "error": "自定义设备适配器未初始化"}
        try:
            result = _custom_led_adapter.discover(
                name=str(body.get("name", ""))[:64],
                room=str(body.get("room", "自定义设备"))[:32],
                device_type=str(body.get("deviceType", body.get("device_type", "light"))),
                manufacturer=str(body.get("manufacturer", "自研"))[:48],
                dev_board=str(body.get("devBoard", body.get("dev_board", "ESP32")))[:48],
            )
            device = result.get("device", {})
            if bool(body.get("register", False)) and device:
                registered = _custom_led_adapter.register_pending(device.get("id", ""))
                if not registered.get("success"):
                    return self._public_custom_payload(registered)
                device = registered.get("device", device)
                self._register_custom_context(device)
            _record_context_event(
                "custom_device_scanned", "完成自定义设备扫描，等待手动接入" if not body.get("register", False) else "发现并注册自定义设备",
                details={"deviceId": device.get("id"), "transport": device.get("transport"),
                         "boardIp": device.get("board_ip")},
                entity_type="device", entity_id=device.get("id"), source="custom_discovery",
            )
            safe_result = dict(result)
            safe_result["device"] = self._public_custom_device(device)
            discovery = dict(safe_result.get("discovery") or {})
            discovery.pop("source", None)
            discovery.pop("board_ip", None)
            discovery["source"] = "local-network-discovery"
            safe_result["discovery"] = discovery
            return self._public_custom_payload(safe_result)
        except Exception as exc:
            log(f"[CUSTOM] LED discovery failed: {type(exc).__name__}")
            return {"success": False, "error": str(exc)[:160]}

    def _register_custom_context(self, device):
        """只有用户确认后，才把扫描结果写入设备注册表和 AI 上下文。"""
        if _intent_engine and device:
            _intent_engine.register_device({
                "id": device.get("id"), "name": device.get("name"),
                "type": "custom", "room": device.get("room"),
                "aliases": [device.get("name", "设备"), "设备", "智能设备"],
                "capabilities": device.get("capabilities", []),
            })
        if _context_engine and device:
            _context_engine.upsert_entity(
                "device", device.get("id"), name=device.get("name", device.get("id")),
                aliases=[device.get("name", "设备"), "设备", "智能设备"],
                capabilities=device.get("capabilities", []),
                state={"type": "custom", "room": device.get("room"),
                       "protocol": device.get("protocol"),
                       "transport": device.get("transport")},
                source="custom_device_register", enabled=True,
            )
            _context_engine.rebuild_snapshot()

    def _register_scanned_custom(self, body):
        if _custom_led_adapter is None:
            return {"success": False, "error": "自定义设备适配器未初始化"}
        device_id = str(body.get("device_id", body.get("id", ""))).strip()
        result = _custom_led_adapter.register_pending(device_id)
        if result.get("success"):
            self._register_custom_context(result.get("device", {}))
            _record_context_event("custom_device_registered", "用户确认接入自定义设备",
                                  details={"deviceId": device_id}, entity_type="device",
                                  entity_id=device_id, source="custom_device_register")
        return self._public_custom_payload(result)

    def _custom_led_status(self, device_id):
        if _custom_led_adapter is None:
            return {"success": False, "error": "自定义设备适配器未初始化"}
        return _custom_led_adapter.status(device_id)

    def _record_custom_led_operation(self, device_id, action, params, result):
        """统一记录自定义设备动作；不把内网 endpoint 写入上下文。"""
        ok = bool(result.get("success"))
        try:
            conn = _db()
            conn.execute(
                "INSERT INTO device_operations(device_id,action,params_json,result,source) VALUES(?,?,?,?,?)",
                (device_id, str(action), json.dumps(params or {}, ensure_ascii=False),
                 "ok" if ok else "failed", "custom_adapter"),
            )
            conn.commit(); conn.close()
        except Exception:
            pass
        _record_context_event(
            "device_operation", f"自定义设备 {device_id} {action}{'成功' if ok else '失败'}",
            details={"action": str(action), "params": params or {}, "success": ok},
            entity_type="device", entity_id=device_id, source="custom_adapter",
            severity="info" if ok else "warning",
        )
        if str(action) not in {"query", "status"}:
            device = _custom_led_adapter.get(device_id) if _custom_led_adapter is not None else {}
            name = device.get("name", "自定义设备") if isinstance(device, dict) else "自定义设备"
            result_text = "成功" if ok else "失败"
            detail = ""
            if isinstance(params, dict) and params.get("color"):
                detail = f"，颜色{params.get('color')}"
            _tts_speak(f"{name}{detail}{'操作' + result_text}", category=f"device:{device_id}")
        return result
    ...
    def _get_log_chart(self, keyword="日志统计", days=7):
        ...
        return {
            ...
            "charts": [
                ...
                {
                    # HistoryPage reads the gauge by this stable id; keeping the
                    # id aligned prevents a valid response from rendering empty.
                    "id": "today_gauge",
                    "title": "今日实时状态",
                    "type": "gauge",
                    "data": {
    ...
    def _build_log_analysis_response(self, keyword: str) -> dict:
        """Analyze persisted counters locally so live-log questions never wait on AI."""
        dashboard = self._chat_chart_data(keyword)
        charts = list(dashboard.get("charts") or [])
        daily = next((item for item in charts if item.get("id") == "daily_activity"), {})
        today = next((item for item in charts if item.get("id") == "today_gauge"), {}).get("data", {})
        dates = list((daily.get("xAxis") or {}).get("data") or [])
        series = {str(item.get("name", "")): list(item.get("data") or []) for item in daily.get("series", [])}
        requests = [int(value or 0) for value in series.get("请求", [])]
        chats = [int(value or 0) for value in series.get("对话", [])]
        operations = [int(value or 0) for value in series.get("设备操作", [])]
        security = [int(value or 0) for value in series.get("安全事件", [])]
        ...
        def trend(values: list[int]) -> str:
            ...
            if delta == 0:
                return "与上一日持平"
            return f"较上一日{'增加' if delta > 0 else '减少'}{abs(delta)}"

        peak_index = requests.index(max(requests)) if requests else 0
        peak_text = f"{dates[peak_index]}请求量最高，为{requests[peak_index]}次" if dates and requests else "暂无七日请求样本"
        today_requests = int(today.get("total_requests", requests[-1] if requests else 0) or 0)
        today_chats = int(today.get("total_chat", chats[-1] if chats else 0) or 0)
        today_operations = int(today.get("total_device_ops", operations[-1] if operations else 0) or 0)
        today_security = int(today.get("total_security_events", security[-1] if security else 0) or 0)
        risk_text = "未发现新增安全事件" if today_security == 0 else f"发现{today_security}条安全事件，需要查看警戒详情"
        reply = (
            "七日与当日日志分析\n"
            f"触发原因：主人要求分析真实日志趋势，本次直接读取D6数据库，没有调用外部大模型。\n"
            f"七日趋势：{peak_text}；最近一日请求{trend(requests)}，对话{trend(chats)}，设备操作{trend(operations)}。\n"
            f"当日状态：请求{today_requests}次、对话{today_chats}次、设备操作{today_operations}次、安全事件{today_security}条；{risk_text}。\n"
            f"设备在线：{int(today.get('online_devices', 0) or 0)}/{int(today.get('total_devices', 0) or 0)}；"
            f"传感器在线：{int(today.get('online_sensors', 0) or 0)}/{int(today.get('total_sensors', 0) or 0)}。\n"
            "建议：趋势图用于发现请求突增、设备操作异常和安全事件；出现异常时再进入警戒详情，不会仅凭请求量自动控制设备。"
        )
        _record_context_event(
            "local_log_analysis", "七日与当日日志分析已生成",
            details={"days": len(dates), "today": today, "peak": peak_text},
            source="local_analytics", severity="info",
        )
        return {
            "reply": reply, "role": "assistant", "source": "local_log_analysis",
            "voiceSequence": [_vs_entry(f"日志分析已完成。今日请求{today_requests}次，设备操作{today_operations}次，{risk_text}。")],
            "chartData": dashboard,
        }
    ...
    def _chat_chart_data_if_useful(self, keyword=""):
        text = str(keyword or "")
        if not any(token in text for token in ("图表", "趋势", "统计", "分析", "状态", "汇总", "日志", "七日", "当日", "历史")):
            return {"charts": []}
        return self._chat_chart_data(text)
    ...
    def _toggle_device(self, device_id, body):
        ...
        if device_id == "door_01" and not bool(is_on):
            r = _close_door_without_password()
        else:
    ...
    def _derive_sleep_plan(self) -> dict:
        """从实时设备状态推导睡眠方案；只提出当前确实需要变化的设备。"""
        ...
        for device in devices:
            ...
            is_on = bool(device.get("isOn", device.get("is_on", False)))
            if is_on:
                active_devices.append(device_id)
            if device_id.startswith("light_") and is_on:
                active_lights.append(device_id)
            if device_id == "curtain_01":
                position = device.get("value", device.get("primaryValue", device.get("primary_value", 0)))
                try:
                    if float(position) > 0:
                        curtain_open.append(device_id)
                except (TypeError, ValueError):
                    pass
        commands = [{"type": "device", "device_id": device_id, "action": "off", "params": {}}
                    for device_id in active_lights]
        commands.extend({"type": "device", "device_id": device_id, "action": "set_position", "params": {"value": 0}}
                        for device_id in curtain_open)
        status = f"当前有 {len(active_devices)} 个设备处于开启状态，其中灯具 {len(active_lights)} 个；窗帘开放 {len(curtain_open)} 个。"
        suggestion = "关闭仍亮着的灯具并合上已打开的窗帘，空调等设备保持现状，避免打断睡眠环境。"
        analysis = "用户表达的是休息目标，不是固定场景名称；系统结合当前实时状态，只提出确有必要的变更，避免误关空调或误操作离线设备。"
        total = max(1, len(devices))
        return {
            "type": "device_plan", "planType": "sleep", "reason": "用户表达了睡眠/休息目标",
            "status": status, "suggestion": suggestion, "analysis": analysis,
            "commands": commands,
            "analysisCharts": [
                {"id": "sleep_status", "title": "睡前设备状态", "type": "bar",
                 "xAxis": {"type": "category", "data": ["开启设备", "亮灯", "窗帘开放"]},
                 "series": [{"name": "数量", "type": "bar", "data": [len(active_devices), len(active_lights), len(curtain_open)]}]},
                {"id": "sleep_mix", "title": "当前设备构成", "type": "pie",
                 "xAxis": {"type": "category", "data": ["需调整", "保持"]},
                 "series": [{"name": "设备", "type": "pie", "data": [len(commands), max(0, total - len(commands))]}]},
                {"id": "sleep_risk", "title": "睡眠风险画像", "type": "radar",
                 "radar": {"indicator": [{"name": "灯光", "max": total}, {"name": "窗帘", "max": total}, {"name": "设备", "max": total}]},
                 "series": [{"name": "当前", "type": "radar", "data": [{"name": "当前", "value": [len(active_lights), len(curtain_open), len(active_devices)]}]}]},
                {"id": "sleep_actions", "title": "建议操作趋势", "type": "line",
                 "xAxis": {"type": "category", "data": ["现状", "建议后"]},
                 "series": [{"name": "需要调整", "type": "line", "smooth": True, "data": [len(commands), 0]}]},
            ],
        }
    ...
    def _chat(self, body):
        ...
        if _is_log_analysis_request(last_text):
            return self._build_log_analysis_response(last_text)
        ...
        # ===== 意图引擎三层解析 =====
        if _intent_engine and not _requires_deep_analysis(last_text):
            # 意图执行器
            def _exec_intent(intent):
                return self._execute_intent(intent, ai_execution=True)

            direct_intent = _direct_device_intent(last_text)
            scene_intent = _prepared_scene_intent(last_text)
            if direct_intent is not None:
                direct_execution = _exec_intent(direct_intent)
                result = {
                    "source": "fast", "intent": direct_intent,
                    "executed": bool(direct_execution and direct_execution.get("success", False)),
            ...
            elif scene_intent is not None:
                scene_execution = self._execute_intent(scene_intent, ai_execution=True)
                result = {
                    "source": "prepared_scene", "intent": scene_intent, "executed": bool(scene_execution.get("success")),
                    "result": scene_execution,
                    "reply": _format_execution_result(str(scene_intent.get("scene_name", "场景")), scene_execution),
                }
            else:
                result = _intent_engine.parse(last_text, execute=True, executor=_exec_intent)

            if result.get("reply") and result.get("source") in ("fast", "fuzzy", "prepared_scene"):
                # 快速匹配或模糊推理命中
                ...
                response = {
                    ...
                    "chartData": {"charts": intent.get("analysisCharts", [])} if intent.get("analysisCharts") else self._chat_chart_data_if_useful(last_text),
                }
                # 附加情感和指代消解信息
        ...
        if commands:
            ...
            if safe_commands:
                batch = self._execute_commands_concurrently(safe_commands, ai_execution=True)
                batch_execution = batch
                exec_results = batch.get("results", [])
                reply = (strip_ai_commands(reply).strip() + "\n\n" +
                         _format_execution_result("AI 设备调整", batch)).strip()
                _record_context_event(
                    "ai_direct_execution", "AI 设备命令已直接并发执行",
                    details={"commands": safe_commands, "execution": batch},
                    source="chat", severity="info" if batch.get("success") else "warning",
                )
        ...
        response = {
            ...
            "chartData": self._chat_chart_data_if_useful(last_text),
        }
        if exec_results:
            response["executed_commands"] = exec_results
            response["executed"] = bool(batch_execution and batch_execution.get("success"))
            response["execution"] = batch_execution
        # 广播AI对话事件
        if _proto_gateway:
    ...
    def _execute_intent(self, intent: dict, ai_execution: bool = False) -> dict:
        """执行意图引擎解析出的意图"""
        itype = intent.get("type", "")

        if itype == "device_plan":
            return self._execute_commands_concurrently(
                _safe_plan_commands(intent.get("commands", [])), ai_execution=True,
            )
        if itype == "scene":
            ...
            return self._door_control({"action": door_action}, ai_execution=ai_execution)
    ...
    def _execute_commands_concurrently(self, commands: list[dict], ai_execution: bool = True) -> dict:
        """并发调用独立设备，保持结果顺序并让单设备失败可见。"""
        safe_commands = _safe_plan_commands(commands)
        return execute_device_commands(
            safe_commands,
            lambda command: self._execute_ai_command(command, ai_execution=ai_execution),
            max_workers=min(4, max(1, len(safe_commands))),
        )

    def _execute_ai_command(self, cmd: dict, ai_execution: bool = False) -> dict | None:
        """执行 AI 回复中解析出的控制指令"""
        ctype = cmd.get("type", "")
        if ctype == "device":
            ...
            if dev_id == "door_01" and action in ("on", "off"):
                return self._door_control({"action": "open" if action == "on" else "close"}, ai_execution=ai_execution)
            if action in ("on", "off"):
    ...
    def _internal_api_call(self, path: str, method: str, params: dict) -> dict:
        ...
        API_MAP = {
            ...
            "/api/ai/automation/catalog": {"GET": self._get_automation_catalog},
            "/api/ai/automation/popups": {"GET": self._get_automation_popups},
            "/api/ai/automation/check": {"POST": self._run_automation_check_api},
            "/api/ai/knowledge/stats": {"GET": lambda: {"success": True, **_device_solution_knowledge.get_stats()}},
            "/api/ai/knowledge/search": {"POST": self._search_device_solutions},
            ...
            "/api/ai/assistant/acknowledge": {"POST": self._acknowledge_assistant_alert},
    ...
    def _door_control(self, body, ai_execution: bool = False):
        action = body.get("action", "query")
        password = body.get("password", body.get("doorPassword"))
        if action == "open" and ai_execution and not password:
            password = _AI_DOOR_PASSWORD
        if action == "query":
            return hw_living_status("door")
        if action not in ("open", "close"):
            return {"error": "门禁 action 仅支持 query/open/close", "code": 400}
        if _door_password_required_for_action(action) and not password:
            return {"error": "门禁操作需要密码", "code": 403}
        if action == "open" and password:
            verified, verify_error = verify_door_password_api(password)
            if not verified:
                client_id = getattr(self, "client_address", ("internal", 0))[0]
                event = _security_anomaly_guard.record_door_password_failure(client_id)
                if event:
                    _publish_security_anomaly(event)
                return {"error": verify_error or "门禁密码错误", "code": 403, "success": False}
        r = _close_door_without_password() if action == "close" else hw_toggle("door_01", True, door_password=password)
        if r.get("success"):
            _apply_commanded_device_state("door_01", action == "open", 0)
        _record_context_event(
            "door_operation",
            f"Door {action} {'succeeded' if r.get('success') else 'failed'}",
            details={"action": action, "passwordProvided": bool(password), "aiExecution": bool(ai_execution), "success": bool(r.get("success"))},
            entity_type="device",
            entity_id="door_01",
            source="api",
            severity="info" if r.get("success") else "warning",
    ...
    def _activate_scene(self, body):
        ...
        if scene_name and not scene_id:
            scene_id = match_prepared_scene(scene_name)
        if not scene_id:
            return {"error": "需要 scene_id 或 name"}
        commands = get_scene_commands(scene_id)
        if not commands:
            return {"error": f"场景 {scene_id} 不存在"}
        execution = self._execute_commands_concurrently(commands, ai_execution=True)
        # 更新数据库
        try:
            conn = _db()
        ...
        # 广播场景激活事件
        if _proto_gateway:
            _proto_gateway.broadcast_event("scene_activated", {"scene_id": scene_id, "name": meta.get("name", ""),
                                                                  "results_count": len(execution.get("results", []))})
        _record_context_event(
            "scene_operation", f"Scene {scene_id} activated",
            details={"sceneId": scene_id, "execution": execution},
            entity_type="scene", entity_id=scene_id, source="api",
            severity="info" if execution.get("success") else "warning",
        )
        return {"scene_id": scene_id, "name": meta.get("name", ""), **execution}

    def _list_scenes(self):
        scenes = []
        for sid, meta in SCENE_META.items():
            actions = get_scene_commands(sid)
            scenes.append({
    ...
    def _ac_control(self, body):
        action = body.get("action", "query")
        if action == "on":
            _security_anomaly_guard.authorize_state("ac_01", True, source="ac_api")
            r = hw_toggle("ac_01", True)
        elif action == "off":
            _security_anomaly_guard.authorize_state("ac_01", False, source="ac_api")
    ...
    def _get_guard_status(self):
        ...
        status["planConfirmationEnabled"] = False
    ...
    def _push_test(self, body):
        if _notification_service is None:
            return {"success": False, "error": "QQ推送服务未初始化"}
        message = str(body.get("message", "A9 QQ 推送链路测试"))[:200]
        ok = _notification_service.send("push_test", "A9推送测试", message, {"destination": "QQ私聊"})
        return {"success": bool(ok), "status": _notification_service.status(),
                "message": "QQ推送已发送" if ok else "QQ推送发送失败，请检查QQ网关"}
    ...
    def _update_guard_config(self, body):
        ...
        updates.pop("planConfirmationEnabled", None)
        updates["planConfirmation"] = {"enabled": False}
    ...
    def _get_automation_catalog(self):
        if _automation_service is None:
            return {"success": False, "rules": [], "total": 0}
        return {"success": True, "rules": _automation_service.engine.catalog, "total": len(_automation_service.engine.catalog)}

    def _search_device_solutions(self, body):
        payload = body if isinstance(body, dict) else {}
        query = str(payload.get("query", "")).strip()
        if not query:
            raise ValueError("query 不能为空")
        try:
            limit = max(1, min(40, int(payload.get("limit", 12))))
        except (TypeError, ValueError):
            limit = 12
        items = _device_solution_knowledge.search(
            query, available_devices=_knowledge_available_devices(), limit=limit,
        )
        return {"success": True, "query": query, "items": items, "total": len(items),
                "catalog": _device_solution_knowledge.get_stats()}

    def _get_automation_popups(self, limit=50):
        if _automation_service is None:
            return {"success": False, "items": []}
        items = _automation_service.outbox.pending(limit=limit)
        return {"success": True, "items": items, "total": len(items)}

    def _run_automation_check_api(self, body=None):
        result = _run_automation_check(manual=True)
    ...
    def _get_assistant_feed(self, limit=30, since=0):
        ...
        for feed_item in items:
            ...
            requires_ack = _assistant_item_requires_acknowledgement(feed_item)
            feed_item["requiresAcknowledgement"] = requires_ack
            feed_item["acknowledged"] = (
                bool(_proactive_intelligence.is_alert_acknowledged(int(feed_item.get("id", 0))))
                if requires_ack else True
            )
        ...
        if _adaptive_guard is not None:
            ...
            guard_candidates = [
                ...
                and incident.get("needsFeedback") and not incident.get("feedbackReceived")
                and float(incident.get("createdTs", incident.get("created_ts", 0)) or 0) > conversation_cutoff
            ]
            for incident in guard_candidates[:1]:
                if (
                    _automation_service is not None
                    and _automation_service.outbox.has_source_incident(int(incident.get("id", 0)))
                ):
                    # The automation receipt contains the same incident with
                    # complete evidence and per-device results.  Never render a
                    # second generic guard card beside it, even after the owner
                    # has acknowledged the popup.
                    continue
                ...
                summaries = {
                    ...
                    "home_absence_device_on": "毫米波连续确认无人，但空调或换气设备仍开启，系统已安全关闭。",
                }
                ...
                projected["requiresAcknowledgement"] = _assistant_item_requires_acknowledgement(projected)
                projected["acknowledged"] = bool(
                    _proactive_intelligence.is_alert_acknowledged(incident_id)
                )
                items.append(projected)
        if _automation_service is not None:
            # 规则引擎弹窗投影到同一助手流；负的大整数 ID 与现有事件空间隔离。
            for popup in _automation_service.outbox.pending(limit=limit):
                popup_id = -1000000000 - int(popup["id"])
                if since and popup_id <= since:
                    continue
                payload = popup.get("payload") or {}
                items.append({
                    "id": popup_id,
                    "kind": "automation_alarm" if popup.get("urgent") else "automation_adjustment",
                    "title": payload.get("eventTitle") or ("安全联动报警" if popup.get("urgent") else "智能调整已执行"),
                    "summary": payload.get("description") or payload.get("triggerReason") or "设备联动已执行。",
                    "severity": "danger" if popup.get("urgent") else "info",
                    "evidence": payload,
                    "operations": payload.get("actions", []),
                    "chart": {},
                    "feedback": popup.get("feedback"),
                    "feedbackEnabled": bool(not popup.get("urgent") and popup.get("feedback") is None),
                    "createdAt": str(popup.get("createdAt", "")),
                    "createdTs": float(popup.get("createdAt", 0) or 0),
                    "requiresAcknowledgement": bool(popup.get("requiresAcknowledgement")),
                    "acknowledged": bool(popup.get("acknowledged")),
                })
    ...
    def _acknowledge_assistant_alert(self, body):
        if _proactive_intelligence is None:
            return {"success": False, "error": "proactive intelligence unavailable"}
        event_id = int(body.get("eventId", body.get("event_id", 0)))
        if event_id <= -1000000000 and _automation_service is not None:
            popup_id = abs(event_id + 1000000000)
            return {"success": _automation_service.outbox.acknowledge(popup_id), "eventId": event_id}
        visible = self._get_assistant_feed(limit=100, since=0).get("items", [])
        event = next((item for item in visible if int(item.get("id", 0)) == event_id), None)
        if not event or not event.get("requiresAcknowledgement"):
            raise ValueError("alert event not found or does not require acknowledgement")
        result = _proactive_intelligence.acknowledge_alert(event_id)
        _record_context_event(
            "assistant_alert_acknowledged", f"Assistant alert {event_id} acknowledged",
            details={"eventId": event_id}, source="assistant", severity="info",
        )
        return result
    ...
    def _submit_assistant_feedback(self, body):
        ...
        if event_id <= -1000000000 and _automation_service is not None:
            popup_id = abs(event_id + 1000000000)
            try:
                result = _automation_service.submit_feedback(
                    popup_id, score, body.get("choice", ""), body.get("note", body.get("notes", "")),
                )
            except (TypeError, ValueError) as exc:
                return {"success": False, "error": str(exc)}
            _record_context_event("automation_feedback", "智能调整反馈已记录", details=result, source="assistant", severity="info")
            return {"success": True, **result}
    ...
    def _get_external_feed(self):
        """Return a ten-minute cached external digest; never blocks device APIs."""
        global _external_digest_cache, _external_digest_at
        if _external_intelligence is None:
            return {"success": False, "items": [], "categories": [], "error": "外部信息服务未初始化"}
        now = time.time()
        with _external_digest_lock:
            if now - _external_digest_at < _EXTERNAL_DIGEST_INTERVAL and _external_digest_cache.get("success"):
    ...
    def _update_external_config(self, body):
        ...
        try:
            payload = body if isinstance(body, dict) else {}
            # 位置、交通源和行情标的由设备端自动获取；前端只能切换外部信息采集开关。
            result = _external_intelligence.update_config({"enabled": payload.get("enabled")})
        except (TypeError, ValueError) as exc:
            return {"success": False, "error": str(exc)}
        global _external_digest_at, _external_digest_cache
        with _external_digest_lock:
            _external_digest_at = 0.0
            if not result.get("enabled"):
                _external_digest_cache = {"success": True, "enabled": False, "categories": [], "items": []}
    ...
    def _confirm_assistant_plan(self, body):
        ...
        if entry.get("status") == "claimed":
            ...
            if payload.get("type") == "ai_commands":
                results = []
                for command in _safe_plan_commands(payload.get("commands", [])):
                    results.append(self._execute_ai_command(command, ai_execution=True))
                result = {"success": all(bool(item and item.get("success", False)) for item in results), "actions": results}
            else:
                result = self._execute_intent(payload, ai_execution=True)
            with _PENDING_PLAN_LOCK:
...
def _initialize_super_context():
    ...
    handlers = {
        ...
        "door_control": lambda action, password=None: service._door_control({"action": action, "password": password}, ai_execution=True),
    }
...
def main():
    global _intent_engine, _proto_gateway, _external_intelligence, _custom_led_adapter
    log("=" * 60)
    ...
    db_init()
    try:
        _custom_led_adapter = CustomLedAdapter(SMART_HOME_DIR / "custom_led_devices.json")
        _adapter_registry.register(_custom_led_adapter)
        log("[CUSTOM] LED 自定义设备适配器已初始化（定向发现，HTTP 文档协议）")
    except Exception as exc:
        _custom_led_adapter = None
        log(f"[CUSTOM] LED 适配器初始化失败: {type(exc).__name__}")
    ...
    log("[EXTERNAL] 外部信息缓存已初始化（失败隔离、十分钟缓存）")
    _init_api_key_db()
    ...
    def _linkage_executor(intent):
        try:
            ...
            if itype == "device_toggle":
                device_id = intent.get("device_id", "")
                expected = bool(intent.get("isOn", True))
                _security_anomaly_guard.authorize_state(device_id, expected, source="linkage_engine")
                return hw_toggle(device_id, expected)
            elif itype == "device_control":
    ...
    threading.Thread(target=_security_state_watch_thread, daemon=True).start()
    ...
    threading.Thread(target=_external_cycle_thread, daemon=True).start()
    log("[PROACTIVE] 十分钟助手汇总线程已启动")
    log("[EXTERNAL] 十分钟外部信息线程已启动（不播报、不进入状态上下文）")
