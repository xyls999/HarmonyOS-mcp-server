...
_CHINESE_SOURCE_DOMAINS = (
    ...
    "amap.com", "baidu.com", "qq.com", "pconline.com.cn", "126.com",
)
...
def chinese_source_name(url: str) -> str:
    ...
    names = {
        ...
        "baidu.com": "百度", "qq.com": "腾讯", "pconline.com.cn": "太平洋电脑网",
        "126.com": "网易",
    }
...
class ExternalIntelligenceCollector:
    def __init__(self, db_path: str, *, fetcher: Callable[[str, int], Any] | None = None,
                 location_fetcher: Callable[[str, int], Any] | None = None,
        ...
        self._custom_location_fetcher = location_fetcher is not None
        self.location_fetcher = location_fetcher or self._fetch_json
        self._init_db()
    ...
    def _init_db(self) -> None:
        ...
        try:
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS external_intelligence_config(
                ...
                market_symbol TEXT NOT NULL DEFAULT '', location_source TEXT NOT NULL DEFAULT '',
                location_confidence TEXT NOT NULL DEFAULT '未知', location_updated_at REAL NOT NULL DEFAULT 0,
                updated_at REAL NOT NULL)""")
            columns = {str(row[1]) for row in conn.execute("PRAGMA table_info(external_intelligence_config)")}
            for name, definition in (
                ("location_source", "TEXT NOT NULL DEFAULT ''"),
                ("location_confidence", "TEXT NOT NULL DEFAULT '未知'"),
                ("location_updated_at", "REAL NOT NULL DEFAULT 0"),
            ):
                if name not in columns:
                    conn.execute(f"ALTER TABLE external_intelligence_config ADD COLUMN {name} {definition}")
            conn.execute(
                "INSERT OR IGNORE INTO external_intelligence_config(id,enabled,updated_at) VALUES(1,1,?)",
                (_now(),),
            )
            # Drop legacy user-entered position/source fields. Only values tagged
            # by the device-side resolver are eligible after this migration.
            conn.execute("""UPDATE external_intelligence_config SET latitude='',longitude='',location_name='',
                traffic_url='',market_symbol='' WHERE location_updated_at=0 AND location_source=''""")
            conn.commit()
        finally:
            conn.close()
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        try:
            row = conn.execute(
                "SELECT enabled,latitude,longitude,location_name,traffic_url,market_symbol,"
                "location_source,location_confidence,location_updated_at,updated_at "
                "FROM external_intelligence_config WHERE id=1"
            ).fetchone()
            if not row:
                return {"enabled": True, "latitude": "", "longitude": "", "locationName": "当前位置",
                        "trafficUrl": "", "marketSymbol": "", "locationSource": "自动获取",
                        "locationConfidence": "未知", "locationUpdatedAt": 0, "updatedAt": 0}
            return {
                "enabled": bool(row[0]), "latitude": str(row[1] or ""), "longitude": str(row[2] or ""),
                "locationName": str(row[3] or "当前位置"), "trafficUrl": str(row[4] or ""),
                "marketSymbol": str(row[5] or ""), "locationSource": str(row[6] or "自动获取"),
                "locationConfidence": str(row[7] or "未知"), "locationUpdatedAt": float(row[8] or 0),
                "updatedAt": float(row[9] or 0),
                "environment": {
                    "weather": bool(os.environ.get("A9_EXTERNAL_WEATHER_URL", "").strip() or
                                    os.environ.get("A9_WEATHER_CITY_CODE", "").strip()),
                    "news": True, "technology": True,
                    "traffic": bool(str(row[4] or "").strip() or
                                    os.environ.get("A9_EXTERNAL_TRAFFIC_URL", "").strip()),
                    "market": bool(os.environ.get("A9_EXTERNAL_MARKET_URL", "").strip()),
                },
            }
        finally:
            conn.close()
    ...
    @staticmethod
    def _valid_location_name(value: Any) -> str:
        text = re.sub(r"\s+", "", str(value or "")).strip()
        if not text or not re.search(r"[\u4e00-\u9fff]", text):
            return ""
        return text[:80]

    def _location_from_payload(self, payload: Any) -> dict[str, str]:
        if not isinstance(payload, dict):
            return {}
        province = self._valid_location_name(payload.get("pro") or payload.get("province"))
        city = self._valid_location_name(payload.get("city") or payload.get("cityName"))
        name = city or province
        if not name:
            addr = self._valid_location_name(payload.get("addr") or payload.get("address"))
            name = addr
        if not name:
            return {}
        return {"locationName": name, "latitude": str(payload.get("lat") or payload.get("latitude") or ""),
                "longitude": str(payload.get("lng") or payload.get("lon") or payload.get("longitude") or "")}

    def _location_from_text(self, payload: Any) -> dict[str, str]:
        text = str(payload or "")
        province = re.search(r"province\s*[:=]\s*[\"']([^\"']+)", text, re.I)
        city = re.search(r"city\s*[:=]\s*[\"']([^\"']+)", text, re.I)
        return self._location_from_payload({"pro": province.group(1) if province else "",
                                             "city": city.group(1) if city else ""})

    def _persist_auto_location(self, location: dict[str, str], source: str, confidence: str) -> dict[str, Any]:
        current = self.get_config()
        latitude = location.get("latitude", "")
        longitude = location.get("longitude", "")
        # 供应商可能只返回城市名；不要把不可信的文本当作坐标。
        ...
        try:
            conn.execute("""UPDATE external_intelligence_config SET latitude=?,longitude=?,location_name=?,
                location_source=?,location_confidence=?,location_updated_at=?,updated_at=? WHERE id=1""",
                         (latitude, longitude, location.get("locationName") or current.get("locationName") or "当前位置",
                          source, confidence, _now(), _now()))
            conn.commit()
        finally:
            conn.close()
        return self.get_config()

    def resolve_location(self, *, force: bool = False) -> dict[str, Any]:
        """设备端自动解析位置；只接受中文供应商结果并持久化来源和置信度。"""
        current = self.get_config()
        if not force and current.get("locationName") and current.get("locationUpdatedAt"):
            if _now() - float(current.get("locationUpdatedAt") or 0) < 86400:
                return current
        env_name = self._valid_location_name(os.environ.get("A9_LOCATION_NAME", ""))
        if env_name:
            return self._persist_auto_location({"locationName": env_name,
                                                "latitude": os.environ.get("A9_LATITUDE", ""),
                                                "longitude": os.environ.get("A9_LONGITUDE", "")},
                                               "设备运行环境", "中")
        if self._custom_fetcher and not self._custom_location_fetcher:
            return current
        providers = (
            ("https://whois.pconline.com.cn/ipJson.jsp?json=true", "太平洋电脑网"),
            ("https://ip.ws.126.net/ipquery", "网易"),
        )
        for url, source in providers:
            if not is_chinese_source_url(url):
                continue
            try:
                raw = self.location_fetcher(url, self.timeout)
                location = self._location_from_payload(raw) if isinstance(raw, dict) else self._location_from_text(raw)
                if location.get("locationName"):
                    return self._persist_auto_location(location, source, "中")
            except Exception:
                continue
        return current

    def _fetch_json(self, url: str, timeout: int) -> Any:
        # Python 标准库的有界 HTTP 请求；Source:
    ...
    def collect(self) -> dict[str, Any]:
        config = self.resolve_location()
        if not bool(config.get("enabled", True)):
            return {"success": True, "enabled": False, "generated_at": datetime.now(_CST).isoformat(),
                    "location": {"name": config.get("locationName", "当前位置"),
                                 "source": config.get("locationSource", "自动获取"),
                                 "confidence": config.get("locationConfidence", "未知")},
                    "categories": [], "items": []}
        categories = ("weather", "traffic", "news", "technology", "market")
        reports = [self.collect_category(category) for category in categories]
        # Round-robin across categories so a full news feed cannot starve
        # technology, weather, traffic or market entries from the assistant.
        buckets = [list(report.get("items", [])) for report in reports if report.get("items")]
        items: list[dict[str, Any]] = []
        while buckets and len(items) < self.max_items:
            next_buckets = []
            for bucket in buckets:
                if bucket and len(items) < self.max_items:
        ...
        return {"success": True, "enabled": True, "generated_at": datetime.now(_CST).isoformat(),
                "location": {"name": config.get("locationName", "当前位置"),
                             "source": config.get("locationSource", "自动获取"),
                             "confidence": config.get("locationConfidence", "未知")},
                "categories": reports, "items": items}
