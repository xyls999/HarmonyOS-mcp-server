...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "feedbackAutomation": {"enabled": False, "minSamples": 3, "minAverageScore": 8.0},
    ...
    "absenceMonitor": {"enabled": False, "minSamples": 3, "cooldownSeconds": 600},
}
...
class AdaptiveGuard:
    _VISIBLE_RULE_KEYS = {
        "kitchen_smoke_or_heat_alarm",
        "environment_high_temperature",
    ...
    def __init__(
        ...
        *,
        ...
        self._environment_streaks: dict[str, int] = {}
        self._environment_confirmed: set[str] = set()
        self.migrate()
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        # 旧版本的总 AI 标志不再作为可运行配置，避免前端误显示为开启。
        config.pop("aiMasterEnabled", None)
        config["activeAiEnabled"] = False
        config["feedbackAutomation"]["enabled"] = False
        config["offlineMonitor"]["enabled"] = False
        config["absenceMonitor"]["enabled"] = False
        return config
    ...
    def _reset_environment_state(self) -> None:
        self._environment_streaks.clear()
        self._environment_confirmed.clear()
    ...
    def _process_snapshot_locked(self, snapshot: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        kitchen_alerts = []
        high_temperature_rooms = {
            str(sensor.get("room", ""))
            for sensor in sensors
            if isinstance(sensor, dict)
            and str(sensor.get("type", "")).lower() in ("temperature", "temp")
            and self._sensor_value(sensor) is not None
            and self._sensor_value(sensor) >= float(self.config["highTemperature"].get("on", 30))
        }
        for sensor in sensors:
            ...
            if sensor_type in ("temperature", "temp") and self.config["highTemperature"].get("enabled", True):
                ...
                if value >= threshold:
                    self._environment_streaks[signature] = self._environment_streaks.get(signature, 0) + 1
                else:
                    self._environment_streaks[signature] = 0
                    self._environment_confirmed.discard(signature)
                if self._environment_streaks.get(signature, 0) >= 3:
                    active_now.add(signature)
                    self._environment_confirmed.add(signature)
                    ...
                    incident = self._record_incident(
                        ...
                        evidence={"sensor": sensor.get("id"), "type": sensor_type, "room": room, "value": value, "threshold": threshold, "confirmSamples": 3},
                        ...
                        message=f"{room}温度连续三次达到{threshold:g}度，已启动空调制冷",
                    )
                    if incident:
                        incidents.append(incident)
            if sensor_type in ("humidity", "humid") and self.config["highHumidity"].get("enabled", True):
                ...
                if value >= threshold and room not in high_temperature_rooms:
                    self._environment_streaks[signature] = self._environment_streaks.get(signature, 0) + 1
                else:
                    self._environment_streaks[signature] = 0
                    self._environment_confirmed.discard(signature)
                if self._environment_streaks.get(signature, 0) >= 3:
                    ...
                    incident = self._record_incident(
                        ...
                        fallback={"deviceId": "ac_01", "action": "fan", "params": {"profile": fallback_profile}},
                        message=f"{room}湿度连续三次达到{threshold:g}%，已启动空调除湿",
                    )
                    if incident:
                        incidents.append(incident)
        ...
        # 固定硬规则模式不执行毫米波无人节能，避免与安全换气和用户手动状态冲突。
        absence_config = {"enabled": False}
    ...
    def list_incidents(self, *, limit: int = 50, pending_only: bool = False) -> list[dict[str, Any]]:
        ...
        keys = ",".join(f"'{key}'" for key in sorted(self._VISIBLE_RULE_KEYS))
        where = f"WHERE rule_key IN ({keys})"
        if pending_only:
            where += " AND status='open' AND needs_feedback=1 AND feedback_received=0"
        with closing(self._connect()) as connection:
            rows = connection.execute(
