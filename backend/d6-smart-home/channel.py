...
def _play_mp3(mp3_path):
    ...
    with _audio_lock:
        try:
            ...
            # Long five-minute summaries can legitimately exceed 15 seconds.
            # Keep the player call bounded, but make the bound long enough for
            # a full report while allowing field tuning without code changes.
            play_timeout = max(15, min(int(_os.environ.get("A9_TTS_PLAY_TIMEOUT", "45")), 120))
            result = _sp.run([oh_play, mp3_path], timeout=play_timeout, capture_output=True, env=env)
