...
def _check_temp_humidity_ac_linkage(sensor_data):
    ...
    if _adaptive_guard is not None:
        return  # The adaptive guard evaluates the complete snapshot after polling.
...
class H(BaseHTTPRequestHandler):
    ...
    def _get_stats(self):
        ...
        with _STATUS_LOCK:
            ...
            stats["linkage_config"] = {k: v.get("enabled", False) for k, v in _LINKAGE_CONFIG.items()}
        return stats
    ...
    def _update_linkage_config(self, body):
        ...
        if _context_engine is not None:
            ...
            _context_engine.rebuild_snapshot()
        return {"success": True, "rule_key": rule_key, "config": _LINKAGE_CONFIG[rule_key]}
