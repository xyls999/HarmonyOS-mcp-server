...
try:
    from central_controller import (
        ...
        all_status,
        # v3: 鉴权与安全
        auth_enabled, auth_key, verify_door_password,
        door_password_required,
        enforce_rate_limit, maybe_security_alarm,
    )
    # Older field controller exposes verify_door_password only; keep the
    # explicit-call contract expected by this bridge without weakening checks.
    ...
    verify_door_password_explicit = lambda c, p=None: False
...
def verify_door_password_api(password=None):
    ...
    if not door_password_required(_CONFIG):
        return True, None
    try:
        result = verify_door_password_explicit(_CONFIG, password)
        return result, None
...
def hw_control(device_id, action, params, door_password=None):
    ...
    try:
        ...
        if device_id == "ac_01":
            ...
            if action in ("cool", "dry", "fan") and profile:
                r = living_text(_CONFIG, "ac", "preset", _TIMEOUT, str(profile))
            elif action in ("cool", "dry", "fan"):
                r = living_text(_CONFIG, "ac", "mode", _TIMEOUT, action)
            elif action == "set_temp":
                r = living_text(_CONFIG, "ac", "temp", _TIMEOUT, str(params.get("value", 26)))
            elif action == "set_mode":
                r = living_text(_CONFIG, "ac", "mode", _TIMEOUT, str(params.get("mode", params.get("value", "cool"))))
            elif action == "set_fan":
                r = living_text(_CONFIG, "ac", "fan", _TIMEOUT, str(params.get("mode", params.get("value", "auto"))))
            else:
                r = living_text(_CONFIG, "ac", "on", _TIMEOUT)
            return _hw_ok(r)

        elif device_id == "light_01":
